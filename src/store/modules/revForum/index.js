...
const getters = {
    ...
    allArray: (state) => (dates = '') => {
        ...
        const aForumsDate = []
        const fCompletedDate = []

        for (const a of formatAForums) {

            aForumsDate.push(a.newDate.slice(0, 10))

        }
        for (const c of formatFCompleted) {

            fCompletedDate.push(c.newDate.slice(0, 10))

        }

        // Extracting dates by task

        const aForumsArrayTask = []
        const fCompletedArrayTask = []

        for (const a of byTaskA) {

            aForumsArrayTask.push(a.newDate.slice(0, 10))

        }
        for (const c of byTaskC) {

            fCompletedArrayTask.push(c.newDate.slice(0, 10))

        }

        // DateInit

        const aForumsInit = aForumsDate.filter(aForum => aForum >= dateInit)
        const fCompletedInit = fCompletedDate.filter(cForum => cForum >= dateInit)

        // DateFin

        const aForumsFin = aForumsDate.filter(aForum => aForum <= dateFin)
        const fCompletedFin = fCompletedDate.filter(cForum => cForum <= dateFin)

        // Task - DateInit

        const aForumsInitTask = aForumsArrayTask.filter(aForum => aForum >= dateInit)
        const fCompletedInitTask = fCompletedArrayTask.filter(cForum => cForum >= dateInit)
        // Task - DateFin

        const aForumsFinTask = aForumsArrayTask.filter(aForum => aForum <= dateFin)
        const fCompletedFinTask = fCompletedArrayTask.filter(cForum => cForum <= dateFin)

        // DateInit - DateFin

        const aForumsInitFin = aForumsDate.filter(aForum => aForum >= dateInit && aForum <= dateFin)
        const fCompletedInitFin = fCompletedDate.filter(cForum => cForum >= dateInit && cForum <= dateFin)

        // DateInit - DateFin - Task
        ...
        if (dateInit === '' && dateFin === '' && taskDate === '') {
            ...
            state.allArray = [formatAForums.length, formatFCompleted.length]

            // Segundo gráfico

            const total = (formatFCompleted.length * 100) / formatAForums.length
            ...
            for (const fC of formatFCompleted) {
            ...
            for (const n of formatFCompleted) {
            ...
            return [state.allArray, state.allPercent, state.userArray, state.userRepeat, state.forumQuality, state.answersArray, state.noAnswers, state.artTask]
        ...
        if (dateInit === '' && dateFin === '' && taskDate) {
            ...
            state.allArray = [byTaskA.length, byTaskC.length]

            // Segundo gráfico

            const total = (byTaskC.length * 100) / byTaskA.length
            const rest = 100 - total
            ...
            return [state.allArray, state.allPercent, state.userArray, state.userRepeat, state.forumQuality]

        }

        if (dateInit && dateFin === '' && taskDate === '') {

            state.allArray = [aForumsInit.length, fCompletedInit.length]
        ...
        if (dateInit && dateFin === '' && taskDate) {
            ...
            state.allArray = [aForumsInitTask.length, fCompletedInitTask.length]
        ...
        if (dateInit === '' && dateFin && taskDate === '') {
            ...
            state.allArray = [aForumsFin.length, fCompletedFin.length]
        ...
        if (dateInit === '' && dateFin && taskDate) {
            ...
            state.allArray = [aForumsFinTask.length, fCompletedFinTask.length]
        ...
        if (dateInit && dateFin && taskDate === '') {
            ...
            state.allArray = [aForumsInitFin.length, fCompletedInitFin.length]
        ...
        if (dateInit && dateFin && taskDate) {
            ...
            state.allArray = [aForumsTIF.length, fCompletedTIF.length]
